...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{Chapitre1/Figures/bact_circ.jpeg}
	\caption{Images de trajectoires de E. coli issues de l'étude \textbf{Citer XX}}
	\label{fig:bact_circ}
\end{figure}
...
\paragraph{}Dans cet exemple, pour être plus précis, la  vitesse $v$ correspond en fait à la moyenne de la vitesse de la kinésine sur le trajet. En effet, si l'on étudie expérimentalement le trajet d'une myosine sur un filament d'actine (voir \autoref{fig:myosine}), on se rend compte que son déplacement n'est pas du tout uniforme mais se fait plutôt par pas : elle alterne des phases de pause immobile ($v=0$) et des avancées ponctuelles ($v>0$). Pour spécifier cette notion de moyenne sur le trajet, on préférera alors utiliser la notation $\langle v \rangle$.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{Chapitre1/Figures/myosine.png}
	\caption{Évolution de la coordonnée d'une myosine sur un filament d'actine au cours du temps (c'est la représentation graphique de l'équation horaire $x(t)$). \textbf{Citer XX}}
	\label{fig:myosine}
\end{figure}
...
\paragraph{}Afin de calculer une vitesse, il est donc essentiel de maîtriser ses dérivées. Les dérivées de fonctions usuelles sont répertoriées dans le \autoref{tab:der}. Si vous n'êtes pas à l'aise avec cet outil, il est conseillé de reprendre des cours de lycée.
