...
\noindent avec $\vv{g}$ l'accélération de la pesanteur, de norme $g = \SI{9,8}{\meter\per\second\squared}$, et dirigée vers le sol. Sur la \textbf{Fig XX} on représente le poids de différentes balles lancées en l'air. En premier approximation, le poids est une force qui ne varie ni dans l'espace, ni dans le temps. Tout au long du mouvement, un objet est soumis au même poids.
...
\paragraph{}En pratique, on fait rarement face à des problèmes faisant intervenir des ressorts à proprement parler. Toutefois, dans bien des cas, il est très pratique de modéliser certains objets par des ressorts. \textbf{Exemple du microtubule et de la pince optique}
...
\paragraph{}La poussée d'Archimède est donc une force antagoniste de la gravitéSi on prend l'exemple de la balle remplie d'air immergé dans l'eau de la \textbf{Fig XX}, on a $\rho_f > \rho$ et donc la poussée d'Archimède est orientée dans le sens opposé de l'accélération de la pesanteur. Cette force retranscrit donc bien le fait qu'il est difficile d'immerger la balle entièrement dans l'eau !
...
\paragraph{}Cette force est donc d'autant plus intense que les charges sont grandes et d'autant plus faible qu'elles sont éloignées. Son sens, dépend lui du signe des deux charges (voir \textbf{Fig XX}). Si les deux charges sont de même signe, alors $q_1q_2 > 0$ et donc la force est attractive. Si les deux charges sont de signes opposés, alors $q_1q_2<0$ et donc la force est répulsive.
...
\subsection{Mesure de force grâce aux pinces optiques}
