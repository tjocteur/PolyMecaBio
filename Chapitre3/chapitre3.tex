...
\textbf{Peut-être à retravailler un poil}
...
\noindent $m$ étant une constante on a en fait $\frac{\mathrm{d}}{\mathrm{d}t}\left( \frac{1}{2}mv^2 \right) =\frac{\mathrm{d}E_c}{\mathrm{d}t} = \vv{F}\cdot\frac{\mathrm{d}\vv{OM}}{\mathrm{d}t}$ et donc en intégrant entre un instant $t_A$ et un instant $t_B$ :

\begin{equation}
	E_{c, B} - E_{c, A} = \int_{t_A}^{t_B}\vv{F}\cdot \mathrm{d}\vv{OM}
\end{equation}

\paragraph{}On définit alors $W_{AB} = \int_{t_A}^{t_B}\vv{F}\cdot \mathrm{d}\vv{OM}$ le travail fourni par la force $\vv{F}$ à l'objet. Le travail est une énergie (qui s'exprime donc aussi en \SI{}{\joule}) qui mesure en quelques sortes l'effort fourni par une force. Si l'on considère une force constante, alors on a simplement :
...
\paragraph{}Pour une force constante opérant pendant une durée $\Delta t$, la relation entre le travail $W$ de cette force et sa puissance $\mathcal{P}$ est simplement :
...
\paragraph{}Une pomme de masse $m=\SI{100}{\g}$ tombe d'un arbre d'une hauteur $h=\SI{1.0}{\meter}$. On néglige les frottements de l'air sur la pomme pendant la chute. Quelle est la vitesse $v$ de la pomme avant son impact avec le sol ? On définira un axe $(Oz)$ orienté du sol vers le ciel pour repérer le mouvement de la pomme.

\paragraph{}Pour répondre à cette question, on peut utiliser le PFD et intégrer pour obtenir la trajectoire et la vitesse de la pomme au point d'impact. Cette méthode est assez longue et fastidieuse. À la place, on peut simplement utiliser le théorème de l'énergie cinétique. La seule force travaillant est le poids $\vv{P} = -mg\vv{e_z}$, et ce sur le trajet de chute représenté par le vecteur déplacement $\vv{\Delta OM} = -h \vv{e_z}$. Ainsi le travail reçu par la pomme est :

\begin{equation}
	W = (-mg\vv{e_z})\cdot (-h\vv{e_z})=mgh
\end{equation}

\noindent La pomme étant immobile au début de la chute, le théorème de l'énergie cinétique donne ici :

\begin{equation}
	\Delta E_c = E_{c, f} - E_{c, i} = E_{c, f} = \frac{1}{2}mv^2 = mgh
\end{equation}

\noindent soit $v=\sqrt{2gh}=\SI{4.4}{\meter\per\second}$.
...
\paragraph{}En effet, \textbf{un corps soumis uniquement à des forces conservatives voit son énergie mécanique conservée}. C'est à dire que tout au long du mouvement on a $E_m = \text{cste}$. Cette propriété peut s'avérer très utile pour résoudre certains problèmes.

\begin{exemple}
	\paragraph{}On pend une masse $m=\SI{1}{\kg}$ au bout d'un ressort de raideur $k$ et de longueur à vide $l_0=\SI{5}{\cm}$, disposé à la vertical, et on tire la masse de telle manière que la longueur du ressort soit $l=\SI{10}{\cm}$ avant de la lâcher sans vitesse initiale à $t=\SI{0}{\second}$. En utilisant le principe de conservation de l'énergie mécanique, calculer la vitesse de la masse lorsque $l=\SI{6}{\cm}$.
\end{exemple}
...
\begin{exemple}
	\paragraph{}Reprendre le problème précédent en supposant que les frottements de l'air avec la masse pendant le mouvement ont exercé un travail résistant de $W=\SI{10}{\joule}$.
\end{exemple}
...
\paragraph{}Le principe de conservation de l'énergie mécanique peut s'avérer très pratique pour étudier le mouvement d'un corps soumis uniquement à des forces conservatives dont on connaît l'énergie potentielle totale associée. Considérons par exemple un snowboardeur évoluant sur une piste, repéré par la coordonnée $x$ et dont le profil est donné sur la partie gauche de la \autoref{fig:snow}.

\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{Chapitre3/Figures/piste.pdf}
	\caption{Profil d'énergie potentielle d'un snowboardeur sur une piste.}
	\label{fig:snow}
\end{figure}


\paragraph{}Celui-ci n'est soumis qu'à son poids en termes de force conservative\footnote{On néglige ici les frottements et la réaction du support ne travaille pas.}, dont on a vu que l'énergie potentielle associée est proportionnelle à l'altitude. Ainsi, on a le profil d'énergie potentielle donné sur la partie droite de la \autoref{fig:snow}.

\paragraph{}On suppose que le snowboardeur part du début de la piste en $x=0$ avec une vitesse nulle. Son énergie cinétique est donc nulle $E_c=0$ et son énergie mécanique totale égale à son énergie potentielle initiale. On peut donc la représenter sur la partie droite de la \autoref{fig:snow}. Lorsque le snowboardeur se laisse glisser, son énergie cinétique augmente et son énergie potentielle diminue de telle manière que son énergie mécanique reste constante. Grâce à ce graphe, on peut calculer la vitesse du snowboardeur et les zones auxquelles il peut accéder en glissant sans trop de calcul. En effet, il est possible de lire directement l'énergie cinétique sur ce graphe comme $E_c = E_m - E_p$. De plus, étant donné que l'on a nécessairement $E_m \geq E_p$ car l'énergie cinétique est une grandeur positive, on a $E_p \leq E_m$. Ainsi, le snowboardeur ne pourra pas atteindre les zones se situant au-dessus de la droite $E_p = E_m$. Dans le cas de la \autoref{fig:snow}, il ne pourra donc jamais franchir la bosse.

\subsection{Électron lié à un atome}

\paragraph{}On peut appliquer le même raisonnement à des systèmes plus complexes dont on connpait le profil d'énergie potentielle. Par exemple, dans l'atome d'hydrogène, l'électron est soumis à des forces d'interaction avec le noyau résultant en l'énergie potentielle dont le profil est donné sur la \autoref{fig:electron}, avec $r$ la distance entre le noyau et l'électron.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{Chapitre3/Figures/electron.pdf}
	\caption{Profil d'énergie potentielle de l'électron dans l'atome d'hydrogène.}
	\label{fig:electron}
\end{figure}

\paragraph{}Infiniment loin du noyau, l'énergie potentielle d'interaction est égale à $E_\infty$. Infiniment proche du noyau, l'énergie potentielle de d'interaction est infinie, cette zone est donc physiquement inaccessible à l'électron (car il faudrait une énergie mécanique infini pour pouvoir y accéder). En fonction de l'énergie totale de l'électron, on peut tirer de ce diagramme plusieurs informations :

\begin{itemize}
	\item L'énergie minimale de l'électron dans l'atome d'hydrogène est $E_\text{min}$.
	\item Pour $E_m < E_\infty$, l'électron est lié au noyau, il ne peut pas s'en éloigner arbitrairement loin. On parle d'état lié de l'électron.
	\item Pour $E_m > E_\infty$, l'électron possède assez d'énergie pour "fuir" l'attraction du noyau et donc partir infiniment loin. On parle d'état libre de l'électron.
\end{itemize}
...
\paragraph{}Il existe de nombreux autres cas physiques possédant des profils d'énergie potentielle non-monotones présentant ces mêmes notions d'états liés et d'états libres. C'est notamment le cas de certains systèmes d'astres en gravitation ou alors d'organelles piégées dans une pince optique (voir TD).
...
\begin{itemize}
	...
	\item Calculer la puissance d'une force.
	\item Faire le lien entre travail et puissance dans le cas de forces constantes.
	\item Utiliser le théorème de l'énergie cinétique pour déterminer la vitesse finale d'un objet à partir du travail d'une force.
	\item Utiliser le théorème de l'énergie cinétique pour déterminer le travail d'une force à partir de l'état initial et l'état final d'un objet.
	\item Déterminer la force associée à une énergie potentielle.
	\item Connaître les énergies potentielles associées aux forces usuelles.
	\item Connaître les conditions d'application du principe de conservation de l'énergie mécanique.
	\item Utiliser la conservation de l'énergie mécanique pour caractériser le mouvement d'un objet dans un profil d'énergie potentielle donné. 
\end{itemize}